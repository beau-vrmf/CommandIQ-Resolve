...
export function NoteDialog({ open, initial, onClose, onSave }: Props) {
  ...
  // Append finalized speech chunks to the current textarea contents.
  const appendFinal = useCallback((chunk: string) => {
    const trimmed = chunk.trim()
    if (!trimmed) return
    setValue((prev) => {
      if (!prev) return trimmed
      // Add a space (or newline if the previous chunk ended a sentence).
  ...
  // Stop listening if the dialog is closed mid-dictation.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
          ...
          {isSupported && (
            ...
            >
              {isListening ? (
                <>
                  <span
                    className="w-2.5 h-2.5 rounded-full bg-rose-200 animate-pulse"
                    aria-hidden="true"
                  />
        ...
        {/* Live interim preview while the engine is mid-thought. */}
        {isListening && (
        ...
        {error && (
          <p className="text-xs text-rose-300">
            ...
            {error === 'not-allowed' &&
              ' — microphone permission was denied. Allow it in your browser settings to dictate.'}
            {error === 'network' &&
        ...
        {!isSupported && (
          <p className="text-xs text-slate-500">
            Dictation isn’t available in this browser. Try Chrome, Edge, or Safari on
            iOS 14.5+.
          </p>
        )}
