  ...
  useEffect(() => {
    async function load() {
      ...
      const [questions, responses] = await Promise.all([
        getQuizQuestions(d.procedure.id),
        getQuizResponses(submissionId),
      ])
      setQuizQuestions(questions)
      const respMap = new Map<string, OjtQuizResponse>()
      for (const r of responses) respMap.set(r.question_id, r)
      setQuizResponses(respMap)
    }
    ...
    <div className="px-4 py-5 max-w-2xl mx-auto w-full">
      ...
      {quizQuestions.length > 0 && (
        <div className="mb-5">
          ...
          {(() => {
            const graded = quizQuestions.filter((q) => q.question_type !== 'short_answer')
            const correct = graded.filter((q) => quizResponses.get(q.id)?.is_correct === true)
            const score = graded.length > 0
              ? Math.round((correct.length / graded.length) * 100)
              : null
            const passing = procedure.quiz_passing_score
            return (
              <>
                {score != null && (
                  <div className={`mb-3 px-3 py-2 rounded-lg text-sm font-medium ${
                    passing == null || score >= passing
                      ? 'bg-green-900/30 border border-green-700 text-green-300'
                      : 'bg-amber-900/30 border border-amber-700 text-amber-300'
                  }`}>
                    Score: {score}%
                    {passing != null && ` (passing: ${passing}%)`}
                    {' — '}{correct.length}/{graded.length} correct
                  </div>
                )}
                <div className="flex flex-col gap-2">
                  {quizQuestions.map((q, idx) => {
                    ...
                    return (
                      <div key={q.id} className="p-3 bg-slate-800 border border-slate-700 rounded-xl">
                        <div className="flex items-start gap-2">
                          <span className="text-xs font-mono text-slate-500 flex-shrink-0 mt-0.5">Q{idx + 1}</span>
                          <div className="flex-1">
                            ...
                            {resp?.is_correct === false && q.correct_answer && (
                              <p className="text-xs text-slate-500 mt-0.5">
                                Correct: {q.correct_answer}
                              </p>
                            )}
                            {!resp && <p className="text-xs text-slate-600 mt-0.5">Not answered</p>}
                          </div>
